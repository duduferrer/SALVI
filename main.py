...
def start(message):
    ...
    if user_already_exists(message):
            bot.send_message(message.chat.id,
    '''
        Eh maboy, tu já tá cadastrado. Se precisar de alguma ajuda, chama o /supervisooor.
    ''')
    else:
        res = bot.send_message(message.chat.id,
'''
...
def unscripted_message(message):
    if user_already_exists(message):
        logging.info("User already exists, random message")
        bot.send_message(message.chat.id,
'''
Não saquei o que tu precisa. Se precisar de alguma ajuda, chama o /supervisooor.
'''     )
    else:
        logging.info("Redirect to start, random message")
        bot.send_message(message.chat.id,
'''
Fala maboy, suave?
